...
describe WorksController do
  ...
  describe "collected" do
    ...
    before do
      @unrestricted_work = create(:posted_work,
                                  authors: [collected_user.default_pseud],
                                  fandom_string: collected_fandom.name)
      @unrestricted_work_in_collection = create(:posted_work,
                                                authors: [collected_user.default_pseud],
                                                collection_names: collection.name,
                                                fandom_string: collected_fandom.name)
      @unrestricted_work_2_in_collection = create(:posted_work,
                                                  authors: [collected_user.default_pseud],
                                                  collection_names: collection.name,
                                                  fandom_string: collected_fandom2.name)
      @restricted_work_in_collection = create(:posted_work,
                                              restricted: true,
                                              authors: [collected_user.default_pseud],
                                              collection_names: collection.name,
                                              fandom_string: collected_fandom.name)
