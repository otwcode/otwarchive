...
describe Admin::PreferencesController do
  ...
  describe "GET #show" do
    ...
    context "when logged in as admin" do
      ...
      it "denies access to other's pages" do
        ...
        it_redirects_to_with_error(admins_path, "Sorry, you don't have permission to access the page you were trying to reach.")
      end
    end
  end
  ...
  describe "DELETE #totp_disable" do
    let(:admin) { create(:admin, otp_required_for_login: true) }
    let(:other_admin) { create(:admin) }

    it "denies access to guest users" do
      delete :totp_disable, params: { admin_id: admin.login }
      it_redirects_to_with_notice(root_url, "I'm sorry, only an admin can look at that area")
    end
    ...
    context "when logged in as admin" do
      ...
      it "allows disabling TOTP" do
        ...
        delete :totp_disable, params: { admin_id: admin.login }
        admin.reload
        expect(admin.otp_required_for_login).to be_falsey
        expect(admin.otp_secret).to be_nil
        expect(admin.otp_backup_codes).to be_empty
        it_redirects_to_with_notice(admin_preferences_path, "TOTP two-factor authentication successfully disabled.")
      end

      it "denies access when TOTP is disabled" do
        fake_login_admin(other_admin)
        delete :totp_disable, params: { admin_id: other_admin.login }
        it_redirects_to_with_error(admin_preferences_path, "TOTP two-factor authentication is already disabled.")
      end
