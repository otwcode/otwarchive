...
describe WorkSearchForm, work_search: true do
  ...
  describe "#set_sorting" do
    it "does not override provided sort column" do
      options = { sort_column: "creators_to_sort_on" }
      searcher = WorkSearchForm.new(options)
      expect(searcher.options[:sort_column]).to eq("creators_to_sort_on")
    end
  ...
  describe "sorting results" do
    describe "by authors" do
      ...
      it "returns all works in the correct order of sortable pseud values" do
        ...
        work_search = WorkSearchForm.new(sort_column: "creators_to_sort_on")
        expect(work_search.search_results.map(&:creators_to_sort_on)).to eq sorted_pseuds_asc

        work_search = WorkSearchForm.new(sort_column: "creators_to_sort_on", sort_direction: "asc")
        expect(work_search.search_results.map(&:creators_to_sort_on)).to eq sorted_pseuds_asc

        work_search = WorkSearchForm.new(sort_column: "creators_to_sort_on", sort_direction: "desc")
        expect(work_search.search_results.map(&:creators_to_sort_on)).to eq sorted_pseuds_asc.reverse
      end
    end
    ...
    describe "by authors who changes username" do
      ...
      it "returns all works in the correct order of sortable pseud values" do
        work_search = WorkSearchForm.new(sort_column: "creators_to_sort_on")
        expect(work_search.search_results.map(&:creators_to_sort_on)).to eq %w[cioelle ruth]
        ...
        work_search = WorkSearchForm.new(sort_column: "creators_to_sort_on")
        expect(work_search.search_results.map(&:creators_to_sort_on)).to eq %w[ruth yabalchoath]
      end
    end
    ...
    it "keeps sort order of tied works the same when work info is updated" do
      ...
      q = WorkQuery.new(sort_column: "creators_to_sort_on", sort_direction: "desc")
