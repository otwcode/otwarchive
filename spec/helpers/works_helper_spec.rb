...
describe WorksHelper do
  ...
  describe "#og_title_description" do
    ...
    context "work title and byline are within character limit" do
      it "returns the work title first" do
        expect(helper.og_title_description(work)).to eq("#{work.title} by #{user.default_pseud.byline}")
      end
    end
    ...
    context "work title exceeds character limit" do
      it "returns the byline first" do
        ...
        expect(helper.og_title_description(work)).to eq("#{user.default_pseud.byline}: #{work.title}")
      end
    end
  end

  describe "#og_meta_description" do
    ...
    context "work summary is present" do
      it "returns the work summary" do
        expect(helper.og_meta_description(work)).to eq(work.summary)
      end
    end
    ...
    context "work summary is absent" do
      ...
      it "returns fandom, relationship and character tags" do
        expect(helper.og_meta_description(work)).to eq("Fandom, Original Work, abc/def, abc, def, xyz")
      end

      context "relationship tags are absent" do
        it "gracefully returns fandom and character tags only" do
          work.relationship_string = ""
          expect(helper.og_meta_description(work)).to eq("Fandom, Original Work, abc, def, xyz")
        end
      end
    end
  end
