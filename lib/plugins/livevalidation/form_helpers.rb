  ...
  module Helpers
    module FormHelper
      [ :text_field, :text_area, :password_field ].each do |field_type|
        ...
        #
        # `alias_method_chain` is deprecated in favor of module#prepend,
        # specifically because this pattern goes against emerging best
        # practices. But in this case, just using the pattern that
        # `alias_method_chain` was meant to abstract is simpler.
        alias_method "#{field_type}_without_live_validations".to_sym, field_type
        alias_method field_type, "#{field_type}_with_live_validations".to_sym
