...
namespace :massimport do
  ...
  desc "Import the 852 Prospect Sentimal archive"
  task(:ssa => :environment) do
    
    @archivist_login = "852_archivist"
    ...
    automated_archive_rescue(:archivist_login => @archivist_login, :base_url => base_url, :author_email => author_email, 
      ...
      :archive_file => archive_file, :story_parse_method => :parse_content_from_852_file, :load_comments => false)
  end  
  ...
  def parse_content_from_ssa_file(work_params, story)
    ...
    warnings = work_params[:relationship_string] || ""
    warnings.split(", ").each do |pairing|
      case pairing
      #when 'J/B'
      #  category_string += "F/F,"
      when 'J/B'
