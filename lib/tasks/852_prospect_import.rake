...
# - uncomment call to send_external_author_invites in automated_archive_rescue_852
# - uncomment line "storyfile = options[:base_dir] + "archive/#{location}.#{filetype}"" and disable following line
...
namespace :massimport852 do
  ...
  def get_base_dir_852(base_dir_default = "/var/www/otwarchive/current/tmp/archive_import/")
  ...
  def load_automated_archive_db_852(filename, attrs_to_keep = [], attribute_name_mapping = {}, value_mapping = {}, values_to_set = {})
    db = Array.new
  ...
  def load_automated_archive_comments_for_work(work, base_dir, location, comment_parse_method)
    ...
    self.send(comment_parse_method, base_dir, location).each do |comment|
      ...
      if comment_object.save
        ...
        message("We ran into errors loading a comment: " + comment_object.errors.full_messages.join(', '))
      end
    end
    message("Loaded #{comment_count} comments")
  end
  ...
  ###
  def load_automated_archive_works_852(archivist, db, options = {})
    ...
    message("Running load_automated_archive_works_852")
    work_ids = []
    errors = []
  ...
  ## :
  def automated_archive_rescue_852(options = {})
    ...
    # load db
    db = load_automated_archive_db_852(base_dir + options[:archive_file], options[:attrs_to_keep], options[:attr_mapping], options[:val_mapping], options[:values_to_set])

    # create works and comments
    work_ids = load_automated_archive_works_852(archivist, db, :base_url => options[:base_url], :base_dir => base_dir,
  ...
  task(:wipe_and_reload, [:send_invites] => :environment) do |t, args|
    ...
    Rake::Task['massimport852:clear_collection_testing_only'].invoke
    Rake::Task["massimport852:prospect"].invoke(args.send_invites)
  end
  ...
  # 852 categories and story files has very basic story files
