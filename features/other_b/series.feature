...
Feature: Create and Edit Series
  ...
  Scenario: Users can co-create a series with pseuds that is ambiguous even if they don't allow cocreation
    Given basic tags
   ...
   Then I should see "There are 3 valid users. Please choose the ones you want:"
      And I select "disco (Burnham)" from "series[author_attributes][ambiguous_pseuds][]"
      And I select "Me (myself)" from "series[author_attributes][ambiguous_pseuds][]"
      And I press "Save"
   Then I should see "Series was successfully updated."
      And "testuser" should be the creator of the series "Gentleman Jack"
      And "Me (myself)" should be the creator of the series "Gentleman Jack"
      And "disco (Burnham)" should be the creator of the series "Gentleman Jack"
