...
Scenario: Admin can set an caution support notice
  Given there are no support notices
...
Scenario: Admin can delete a support notice and it will no longer be shown to users
  Given there are no support notices
    And an admin creates an active "notice" support notice
  When I am logged in as a "support" admin
    And I am on the admin_support_notices page
    And I follow "Delete"
...
Scenario: Only latest active support notice will be shown to users
  Given there are no support notices
    And an admin creates an active "caution" support notice
