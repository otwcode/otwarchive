...
Feature: Bookmark Indexing
  ...
  @new-search
  Scenario: Decanonizing a tag should remove its bookmarks from it's metatag's 
  bookmark listings; canonizing it again should not re-add it to the listings
    ...
    When I edit the tag "Amy Sykes"
      ...
      And I press "Save changes"
      And all indexing jobs have been run
      And I go to the bookmarks tagged "Amy"
    Then I should not see "BookmarkedWork"
      And I should not see "BookmarkedSeries"
      And I should not see "BookmarkedExternalWork"
    When I edit the tag "Amy Sykes"
  ...
  @new-search
  Scenario: Decanonizing a tag should not remove its bookmarks from its 
  metatag's bookmarks listings if the bookmarks are also tagged with a different
  subtag
    Given I am logged in as a tag wrangler
      And a canonical character "Amy"
      And a canonical character "Amy Sykes"
  ...
  @new-search
  Scenario: Decanonizing a tag should not remove its bookmarks from its
  metatag's bookmark listings if the bookmarks are also tagged with the metatag
    Given I am logged in as a tag wrangler
      And a canonical character "Amy"
      And a canonical character "Amy Sykes"
      And "Amy" is a metatag of the character "Amy Sykes"
