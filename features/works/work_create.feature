...
#      Then I should see "cosomeone" in the autocomplete
  ...
  Scenario: Users can co-create a work with a co-creator who has multiple pseuds
    ...
    When I go to myself's pseuds page
      ...
      And I press "Create"
    When I am logged in as "herself" with password "password2"
      And I go to herself's pseuds page
      And I follow "New Pseud"
      And I fill in "Name" with "Me"
      And I check "pseud_is_default"
      And I fill in "Description" with "Something not so cute"
      And I press "Create"
    When I am logged in as "testuser" with password "testuser"
    Then I go to the new work page
      And I should see "Post New Work"
      And I select "Not Rated" from "Rating"
      And I check "No Archive Warnings Apply"
      And I fill in "Fandoms" with "Supernatural"
      And I fill in "Work Title" with "All Hell Breaks Loose"
