...
Feature: Gift Exchange Challenge
  ...
  Scenario: When no potential matches can be generated, the Matching page and notification emails reflect that
    Given a locale with translated emails
      And I create the gift exchange "NoMatches" with the following options
        | value      | minimum | maximum | match |
        | prompts    | 1       | 1       | 1     |
        | fandoms    | 1       | 1       | 1     |
      And the user "moderator" enables translated emails
      And I have added a co-moderator "mod2" to collection "NoMatches"
    When the user "badgirlsdoitwell" signs up for "NoMatches" with the following prompts
        | type    | characters | fandoms  | freeforms | ratings | categories |
        | request |            | the show |           |         |            |
        | offer   |            | the show |           |         |            |
      And the user "sweetiepie" signs up for "NoMatches" with the following prompts
        | type    | characters | fandoms  | freeforms | ratings | categories |
        | request |            | the book |           |         |            |
        | offer   |            | the book |           |         |            |
    When I close signups for "NoMatches"
      And I follow "Matching"
      And I press "Generate Potential Matches"
