...
class WorksEditControllerTest < ActionController::TestCase
  ...
  context "when logged in" do
    ...
    context "when working with your own work" do
      ...
      should_assign_to :chapters, :equals => '[@chapter]'
      should_assign_to :pseuds, :equals => '[@user.default_pseud, @pseud]'
      should_assign_to :selected_pseuds, :equals => '[@pseud.id]'
      should_assign_to :coauthors, :equals => '[]'
      should_assign_to :series, :equals => '[]'
      should "set up form" do
        form = select_form "edit_work_#{@work.id}"
        assert_equal ["_method", "work[rating_string]", "work[warning_strings]", "work[category_string]", "work[fandom_string]", "work[pairing_string]", "work[character_string]", "work[freeform_string]", "work[title]", "work[author_attributes][ids]", "work[author_attributes][coauthors]", "pseud[byline]", "work[summary]", "work[notes]", "work[parent_url]", "work[restricted]", "storyseriescheck", "work[series_attributes][id]", "work[series_attributes][title]", "isWip", "work[wip_length]", "work[chapter_attributes][title]", "work[published_at(3i)]", "work[published_at(2i)]", "work[published_at(1i)]", "work[chapter_attributes][content]", "preview_button", "cancel_button"].sort, form.field_names.sort
      end
      context "with no co-authors" do
        setup { get :edit, :locale => 'en', :id => @work.id, :remove => 'me' }
        should_redirect_to 'new_orphan_path(:work_id => @work.id)'
        should_assign_to :work, :equals => '@work'
      end
