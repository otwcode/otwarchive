class Admin::AdminInvitationsController < ApplicationController
  ...
  def create
    ...
    if @invitation.invitee_email.blank?
      ...
      flash[:notice] = ts("An invitation was sent to %{email_address}", :email_address => @invitation.invitee_email)
  ...
  def invite_from_queue
    ...
    flash[:notice] = ts("%{count} people from the invite queue were invited.", :count => params[:invite_from_queue].to_i)
    redirect_to admin_invitations_url
  end
  ...
  def grant_invites_to_users
    ...
    flash[:notice] = ts('Invitations successfully created.')
    redirect_to admin_invitations_url
  end
  ...
  def find
    ...
    unless @user || @invitation || @invitations
      flash.now[:error] = ts("No results were found. Try another search.")
    end
  end
