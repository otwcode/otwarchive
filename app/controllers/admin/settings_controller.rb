class Admin::SettingsController < ApplicationController
  ...
  def update
    ...
    if params[:admin_setting][:banner_text] != @admin_setting.banner_text && @admin_setting.update_attributes(params[:admin_setting])
      ...
      setflash; flash[:notice] = ts("Setting banner back on for all users. This may take some time")
      redirect_to admin_settings_path
    elsif @admin_setting.update_attributes(params[:admin_setting])
      setflash; flash[:notice] = ts("Archive settings were successfully updated.")
      redirect_to admin_settings_path
    else
      render :action => "index"
    end
