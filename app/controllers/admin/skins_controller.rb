class Admin::SkinsController < ApplicationController
  ...
  def update
    ...
    %w(official rejected cached featured in_chooser).each do |action|
      skins_to_set = params["make_#{action}"] ? Skin.where(id: params["make_#{action}"].map(&:to_i)) : []
      skins_to_unset = params["make_un#{action}"] ? Skin.where(id: params["make_un#{action}"].map(&:to_i)) : []
      skins_to_set.each do |skin|
        ...
        skin.update_attribute(:admin_note, params[:skin_admin_note][skin.id.to_s]) if params[:skin_admin_note] && params[:skin_admin_note][skin.id.to_s]
