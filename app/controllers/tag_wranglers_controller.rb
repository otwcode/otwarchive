class TagWranglersController < ApplicationController
  ...
  def show
    ...
    [Fandom, Character, Relationship, Freeform].each do |klass|
      @counts[klass.to_s.downcase.pluralize.to_sym] = Rails.cache.fetch("/wrangler/counts/sidebar/#{klass.to_s}", race_condition_ttl: 10, expires_in: 1.hour) {klass.unwrangled.in_use.count}
    end
  end
