class SkinsController < ApplicationController
  ...
  def load_skin
    ...
    unless @skin
      setflash; flash[:error] = "Skin not found"
  ...
  def check_editability
    unless @skin.editable?
      setflash; flash[:error] = ts("Sorry, you don't have permission to edit this skin")
  ...
  def check_title
    if params[:skin][:title].match(/archive/i)
      setflash; flash[:error] = ts("You can't use the word 'archive' in your skin title, sorry! (We have to reserve it for official skins.)")
  ...
  def load_archive_parents
    if params[:add_site_parents]
      ...
      if !(skin_parents.uniq & archive_parents.collect(&:id)).empty?
        setflash; flash[:error] = ts("You already have some of the archive components as parents, so we couldn't load the others. Please remove the existing components first if you really want to do this!")
        return true
      end
  ...
  def index
    ...
    if params[:user_id] && @user = User.find_by_login(params[:user_id])
      ...
      if (@user != current_user)
        setflash; flash[:error] = "You can only browse your own skins and approved public skins." 
  ...
  def create
    ...
    if @skin.save
      setflash; flash[:notice] =  ts("Skin was successfully created.")
      if loaded
  ...
  def update
    ...
    if @skin.update_attributes(params[:skin])      
      ...
      setflash; flash[:notice] = ts("Skin was successfully updated.")
      if loaded
        if flash[:error].present?
          setflash; flash[:notice] = ts("Any other edits were saved.")
        else
          flash[:notice] += ts(" We've added all the archive skin components as parents. You probably want to remove some of them now!")
        end
        redirect_to edit_skin_path(@skin)
  ...
  def set
    if @skin.cached?
      setflash; flash[:notice] = ts("The skin %{title} has been set. This will last for your current session.", :title => @skin.title)
      session[:site_skin] = @skin.id
    else
      setflash; flash[:error] = ts("Sorry, but only certain skins can be used this way (for performance reasons). Please drop a support request if you'd like %{title} to be added!", :title => @skin.title)
    end
  ...
  def unset
    ...
    setflash; flash[:notice] = ts("You are now using the default Archive skin again!")
  ...
  def destroy
    ...
    begin
      @skin.destroy
      setflash; flash[:notice] = ts("The skin was deleted.")
    rescue
