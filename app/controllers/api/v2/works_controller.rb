class Api::V2::WorksController < Api::V2::BaseController
  ...
  def create
    ...
    if status == :ok
      ...
      success_reponses, error_responses = works_responses.partition { |r| [:ok, :created, :found].include?(r[:status]) }

      # Send claim notification emails for successful works
      if params[:send_claim_emails] && success_reponses.present?
        notified_authors = notify_and_return_authors(success_reponses.map { |r| r[:work] }, archivist)
      ...
      messages = response_message(messages, success_reponses.present?, error_responses.present?)
    end
