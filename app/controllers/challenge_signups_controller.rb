...
class ChallengeSignupsController < ApplicationController
  ...
  def signup_closed
    setflash; flash[:error] = ts("Sign-up is currently closed: please contact a moderator for help.")
  ...
  def no_signup
    setflash; flash[:error] = ts("What sign-up did you want to work on?")
    redirect_to collection_path(@collection) rescue redirect_to '/'
    false
  end
  ...
  def index
    if params[:user_id] && (@user = User.find_by_login(params[:user_id]))
      if current_user == @user
        ...
        setflash; flash[:error] = ts("You aren't allowed to see that user's sign-ups.")
  ...
  def summary
    if @collection.signups.count < (ArchiveConfig.ANONYMOUS_THRESHOLD_COUNT/2)
      setflash; flash.now[:notice] = ts("Summary does not appear until at least %{count} sign-ups have been made!", :count => ((ArchiveConfig.ANONYMOUS_THRESHOLD_COUNT/2)))
  ...
  def new
    if (@challenge_signup = ChallengeSignup.in_collection(@collection).by_user(current_user).first)
      setflash; flash[:notice] = ts("You are already signed up for this challenge. You can edit your sign-up below.")
  ...
  def create
    ...
    if @challenge_signup.valid? && @challenge_signup.save
      setflash; flash[:notice] = 'Sign-up was successfully created.'
  ...
  def update
    if @challenge_signup.update_attributes(params[:challenge_signup])
      setflash; flash[:notice] = 'Sign-up was successfully updated.'
      redirect_to collection_signup_path(@collection, @challenge_signup)
    else
      render :action => :edit
    end
  ...
  def destroy
    unless @challenge.signup_open || @collection.user_is_maintainer?(current_user)
      setflash; flash[:error] = ts("You cannot delete your sign-up after sign-ups are closed. Please contact a moderator for help.")
    else
      @challenge_signup.destroy
      setflash; flash[:notice] = ts("Challenge sign-up was deleted.")
    end
