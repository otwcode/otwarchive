class ChallengeAssignment < ApplicationRecord
  ...
  belongs_to :pinch_request_signup, class_name: "ChallengeSignup" # TODO Bilka
  ...
  def send_out
    ...
    unless self.sent_at
      ...
      UserMailer.challenge_assignment_notification(collection.id, assigned_to.id, self.id).deliver_later if assigned_to && self.request_signup
    end
  end
