class Comment < ActiveRecord::Base
  ...
  validates_email_veracity_of :email, 
    :message => I18n.t('validation.comments.email', :default => 'does not seem to be valid.'), 
    :timeout => 0.5

  validates_presence_of :content
  validates_length_of :content, 
    :maximum => ArchiveConfig.COMMENT_MAX, 
    :too_long => I18n.t('validation.comments.content_length', :default => "must be less than {{count}} letters long.", :count => ArchiveConfig.COMMENT_MAX)

  def validate
    errors.add_to_base(I18n.t('validation.comments.spam', :default => "This comment looks like spam to our system, sorry! Please try again, or create an account to comment.")) unless check_for_spam
  end
