class CommentObserver < ActiveRecord::Observer
  ...
  def after_commit(comment)
    ...
    original_after_create(comment) if transaction_include_action?(:create)
    original_after_update if transaction_include_action?(:update)
  end

  # Add new comments to the inbox of the person to whom they're directed
  # Send that user a notification email
  def original_after_create(comment)
    comment.reload
  ...
  def original_after_update(comment)
    users = []
    admins = []
