...
class Skin < ActiveRecord::Base
  ...
  IE_CONDITIONS = %w(IE IE5 IE6 IE7 IE8 IE9 IE8_or_lower).freeze
  ROLES = %w(user override).freeze
  ROLE_NAMES = { 'user' => 'add on to archive skin', 'override' => 'replace archive skin entirely' }.freeze
  # We don't show some roles to users
  ALL_ROLES = ROLES + %w(admin translator site)
  DEFAULT_ROLE = 'user'.freeze
  DEFAULT_ROLES_TO_INCLUDE = %w(user override site).freeze
  DEFAULT_MEDIA = ['all'].freeze

  SKIN_PATH = '/stylesheets/skins/'.freeze
  SITE_SKIN_PATH = '/stylesheets/site/'.freeze
  ...
  def valid_public_preview
    return true if official? || !public? || icon_file_name
  ...
  def valid_colors
    if !background_color.blank? && sanitize_css_value(background_color).blank?
      errors.add(:background_color, 'uses a color that is not allowed.')
    end

    if !foreground_color.blank? && sanitize_css_value(foreground_color).blank?
  ...
  def remove_me_from_preferences
    Preference.update_all("skin_id = #{Skin.default.id}", "skin_id = #{id}")
  end

  def editable?
    if filename.present?
      false
    elsif official && public
      return true if User.current_user.is_a? Admin
    elsif author == User.current_user
      true
    else
      false
    end
  ...
  def get_media_for_filename
    (media.nil? || media.empty? ? DEFAULT_MEDIA : media).map do |m|
      if m =~ /max-width: 42em/
        'narrow'
      elsif m =~ /max-width: 62em/
        'midsize'
      else
        m
      end
  ...
  def get_css
    if filename
      File.read(Rails.public_path + filename)
    else
      css
    end
  end
  ...
  def get_style(roles_to_include = DEFAULT_ROLES_TO_INCLUDE)
    Rails.cache.fetch(skin_cache_html_key(self, roles_to_include)) do
      style = ''
      if get_role != 'override' && get_role != 'site'
      ...
      style += get_style_block(roles_to_include)
      style.html_safe
    end
  end
  ...
  def get_ie_comment(style, ie_condition = self.ie_condition)
    if ie_condition.present?
      ie_comment = '<!--[if '
      ie_comment += 'lte ' if ie_condition =~ /or_lower/
      ie_comment += 'gte ' if ie_condition =~ /or_higher/
      ie_comment += 'IE'
      ie_comment += " #{Regexp.last_match(1)}" if ie_condition =~ /IE(\d)/
  ...
  def get_style_block(roles_to_include)
    ...
    if cached?
      ...
      if roles_to_include.include?(get_role)
        if filename.present?
          block += get_ie_comment(stylesheet_link(filename, get_media))
        else
          if (wizard_block = get_wizard_settings).present?
            block += '<style type="text/css" media="' + get_media + '">' + wizard_block + '</style>'
          end
          if css.present?
            block += get_ie_comment('<style type="text/css" media="' + get_media + '">' + css + '</style>')
          end
        end
      end
    end
  ...
  def get_cached_style(roles_to_include)
    block = ''
    self_skin_dir = Skin.skins_dir + skin_dirname
    Skin.skin_dir_entries(self_skin_dir, /^\d+_(.*)\.css$/).each do |sub_file|
      next unless sub_file =~ /^\d+_(.*)\.css$/
      (sheet_role, sheet_media, sheet_ie_condition) = parse_sheet_role(Regexp.last_match(1))
      if roles_to_include.include?(sheet_role)
        block += get_ie_comment(stylesheet_link(SKIN_PATH + skin_dirname + sub_file, sheet_media), sheet_ie_condition) + "\n"
      end
    end
    block
  end
  ...
  def self.naturalized(string)
    string.scan(/[^\d]+|[\d]+/).collect { |f| f =~ /\d+(\.\d+)?/ ? f.to_f : f }
  end
  ...
  def self.load_site_css
    Skin.skin_dir_entries(Skin.site_skins_dir, /^\d+\.\d+$/).each do |version|
      ...
      Skin.skin_dir_entries(version_dir, /^(\d+)-(.*)\.css/).each do |skin_file|
        ...
        skin_role = 'site'
        skin_role = Regexp.last_match(1) if firstline =~ /ROLE: (\w+)/
        skin_media = ['screen']
        if firstline =~ /MEDIA: (.*?) ENDMEDIA/
          skin_media = Regexp.last_match(1).split(/,\s?/)
        elsif firstline =~ /MEDIA: (\w+)/
          skin_media = [Regexp.last_match(1)]
        end
        skin_ie = ''
        skin_ie = Regexp.last_match(1) if firstline =~ /IE_CONDITION: (\w+)/

        full_title = "Archive #{version}: (#{position}) #{title}"
        skin = Skin.find_by_title(full_title)
        skin = Skin.new if skin.nil?
      ...
      top_skin.cache! if %w(staging production).include? Rails.env
    end
  end

  # get the directory name for the skin file
  def skin_dirname
    "skin_#{id}_#{title.gsub(/[^\w]/, '_')}/".downcase
  end
