...
class AdminSetting < ApplicationRecord
  ...
  def self.default_skin
    if self.first
      self.first.default_skin_id ? self.first.default_skin : Rails.cache.fetch(default_skin_cache_key) { Skin.default }
    else
      Rails.cache.fetch(default_skin_cache_key) { Skin.default }
    end
  end
