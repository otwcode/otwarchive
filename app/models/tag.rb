class Tag < ActiveRecord::Base
  ...
  def add_merger_associations
    ...
    ((parents + children) - (new_merger.parents + new_merger.children)).each { |tag| new_merger.add_association(tag) }
    if new_merger.is_a?(Fandom)
      (new_merger.medias - medias).each { |medium| add_association(medium) }
    else
      (new_merger.parents.by_type("Fandom").canonical - fandoms).each { |fandom| add_association(fandom) }
    end
    meta_tags.each { |tag| new_merger.meta_tags << tag unless new_merger.meta_tags.include?(tag) }
    sub_tags.each { |tag| tag.meta_tags << new_merger unless tag.meta_tags.include?(new_merger) }
    mergers.each { |m| m.update_attributes(merger_id: new_merger.id) }
    self.children = []
    self.meta_tags = []
    self.sub_tags = []
  end
