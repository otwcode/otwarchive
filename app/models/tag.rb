class Tag < ApplicationRecord
  ...
  def add_to_autocomplete(score = nil)
    ...
    if self.is_a?(Character) || self.is_a?(Relationship)
      parents.each do |parent|
        REDIS_AUTOCOMPLETE.zadd(ActiveSupport::Inflector.transliterate("autocomplete_fandom_#{parent.name.downcase}_#{type.downcase}"), score, autocomplete_value) if parent.is_a?(Fandom)
      end
    end
    super
  end
  ...
  def remove_from_autocomplete
    ...
    if self.is_a?(Character) || self.is_a?(Relationship)
      parents.each do |parent|
        REDIS_AUTOCOMPLETE.zrem(ActiveSupport::Inflector.transliterate("autocomplete_fandom_#{parent.name.downcase}_#{type.downcase}"), autocomplete_value) if parent.is_a?(Fandom)
      end
    end
  end
  ...
  def remove_stale_from_autocomplete
    ...
    if self.is_a?(Character) || self.is_a?(Relationship)
      parents.each do |parent|
        REDIS_AUTOCOMPLETE.zrem(ActiveSupport::Inflector.transliterate("autocomplete_fandom_#{parent.name.downcase}_#{type.downcase}"), autocomplete_value_before_last_save) if parent.is_a?(Fandom)
      end
    end
  end
