class SkinParent < ActiveRecord::Base
  ...
  validates :position, 
    :uniqueness => {:scope => [:child_skin_id, :parent_skin_id], :message => ts("has to be unique for each parent.")},
  ...
  def no_circular_skin
    ...
    if parent_skin.get_all_parents.include?(child_skin)
      errors.add(:base, ts("^%{child_title} is already one of the parents of %{parent_title}!", :child_title => child_skin.title, :parent_title => parent_skin.title))
    end
  end

  def parent_skin_title
    parent_skin.try(:title)
  end
