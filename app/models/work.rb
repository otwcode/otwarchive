class Work < ApplicationRecord
  ...
  # import URL tracking
  has_one :work_import_url, dependent: :destroy
  ...
  def expire_caches
    ...
    Work.flush_find_by_url_cache if work_import_url.present?
  end
  ...
  def self.flush_find_by_url_cache
    WorkImportUrl.flush_find_by_url_cache
  end

  # Find a work by its imported-from URL, using pre-computed URL variants
  # for efficient exact-match lookups. Delegates to WorkImportUrl.
  def self.find_by_url(url)
    WorkImportUrl.find_work_by_url(url)
  end

  # Read the imported URL from the association (column has been removed).
  def imported_from_url
    work_import_url&.url
  end
