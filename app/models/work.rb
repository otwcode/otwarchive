class Work < ActiveRecord::Base
  ...
  def expire_cache_pseuds
    to_expire = []
    self.pseuds.each do |pseud|
      to_expire << "pseud/#{pseud.id}"
      to_expire << "user/#{pseud.user_id}"
    end
    # at this point we can go asynchronous even if work is being deleted
    # but sadly that's not super helpful
    async(:expire_cached_views, to_expire)
  end
  
  def expire_cache_collections
    to_expire = []
    all_collections = self.all_collections.value_of(:id)
  ...
  def expire_cache_tags
    to_expire = []
    # all the tags and collections with those tags
    (self.taggings.value_of(:tag_id) | self.filter_taggings.value_of(:filter_id)).each do |tag_id|
  ...
  # This is slow so preferably should be run asynchronously via the redis queue
  # (see redis_search_index_queue.rb), but can't always be if the work is being destroyed
  def expire_cache!
    Rails.logger.info "!*!*!*! EXPIRE_CACHE!"
    # collect up the names for everything we need to expire: pseuds, collections, tags
    to_expire = []

    expire_cache_pseuds
    expire_cache_collections
    expire_cache_tags    
  end
  
  # Expire all the relevant index page keys
  # expire the cached pages for the tags on the work and the corresponding filter tags
  # These caches are set in the works controller -- we cache the first n
  # To understand this better read: 
  # http://broadcastingadam.com/2012/07/advanced_caching_revised/
  def expire_cached_views(views_to_expire)
    Rails.logger.info "!*!*!*! views to expire: #{views_to_expire}"
    views_to_expire.uniq.each do |exp|
      ArchiveConfig.PAGES_TO_CACHE.times do |n|
