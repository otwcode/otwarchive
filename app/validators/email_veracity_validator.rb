  ...
  def validate_each(record,attribute,value)
    ...
    unless result
      if options[:allow_blank]
        record.errors[attribute] << (options[:message] ||  t('email_invalid', :default => 'does not seem to be a valid address. Please use a different address or leave blank.'))
      else
        record.errors[attribute] << (options[:message] || t('email_invalid', :default => 'does not seem to be a valid address.'))
      end
    end
  end
end
