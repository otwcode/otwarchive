module CommentsHelper
  ...
  # Comment email helpers

  def email_read_all_comments_link(comment)
    if comment.ultimate_parent.is_a?(Tag)
      text = ts("Read all comments on #{tag_name}", :tag_name => comment.ultimate_parent.name)
      link = {
        :controller => :comments, 
        :action => :index, 
      ...
      text = ts("Read all comments on #{name}", :name => comment.ultimate_parent.commentable_name).html_safe
      link = polymorphic_url(comment.ultimate_parent,
        :view_full_work => true,
        :show_comments => true, 
        :anchor => :comments, 
        :only_path => false)
  ...
  def email_reply_to_comment_link(comment)
    if comment.ultimate_parent.is_a?(Tag)
      link = {
        :controller => :comments, 
        :action => :index, 
      ...
      link = polymorphic_url(@comment.parent, 
        :add_comment_reply_id => @comment.id, 
        :show_comments => true, 
        :anchor => "comment_#{@comment.id}", 
        :only_path => false
    ...
    link_to ts("Reply to this comment"), link
  end

  def email_comment_thread_link(comment)
    if comment.ultimate_parent.is_a?(Tag)
      link = {
        :controller => :comments, 
        :action => :index, 
        :tag_id => @comment.parent, 
        :only_path => false, 
        :anchor => "comment_#{@comment.id}"
      }
    else
    ...
    link_to ts("Go to the thread starting from this comment"), link
  end

  def email_comment_full_thread_link(comment)
    if comment.ultimate_parent.is_a?(Tag)
      link = {
        :controller => :comments, 
        :action => :index, 
    ...
    link_to ts("Go to the thread to which this comment belongs"), link
  end
