class Homepage
  ...
  def admin_posts
    @admin_posts = if Rails.env.development?
                     ...
                     Rails.cache.fetch("home/index/home_admin_posts/v1", expires_in: 20.minutes) do
  ...
  def favorite_tags
    ...
    @favorite_tags ||= if Rails.env.development?
                         ...
                         Rails.cache.fetch("home/index/#{@user.id}/home_favorite_tags/v1") do
                           @user.favorite_tags.to_a.sort_by { |favorite_tag| favorite_tag.tag.sortable_name.downcase }
                         end
                       end
  end
  ...
  def readings
    ...
    @readings ||= if Rails.env.development?
                    ...
                    Rails.cache.fetch("home/index/#{@user.id}/home_marked_for_later/v1") do
                      @user.readings.visible.random_order
                        .limit(ArchiveConfig.NUMBER_OF_ITEMS_VISIBLE_ON_HOMEPAGE)
                        .where(toread: true)
                        .to_a
