class Homepage
  ...
  def admin_posts
    @admin_posts = if Rails.env.development?
                     ...
                     Rails.cache.fetch("v1/home/index/home_admin_posts", expires_in: 20.minutes) do
  ...
  def favorite_tags
    ...
    @favorite_tags ||= if Rails.env.development?
                         ...
                         Rails.cache.fetch("v1/home/index/#{@user.id}/home_favorite_tags") do
                           @user.favorite_tags.to_a.sort_by { |favorite_tag| favorite_tag.tag.sortable_name.downcase }
                         end
                       end
  end
  ...
  def readings
    ...
    @readings ||= if Rails.env.development?
                    ...
                    Rails.cache.fetch("v1/home/index/#{@user.id}/home_marked_for_later") do
                      @user.readings.visible.random_order
                        .limit(ArchiveConfig.NUMBER_OF_ITEMS_VISIBLE_ON_HOMEPAGE)
                        .where(toread: true)
                        .to_a
  ...
  def inbox_comments
    ...
    @inbox_comments ||= @user.inbox_comments.with_bad_comments_removed.for_homepage.for_display
  end
end
