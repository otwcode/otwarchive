...
break;case KEY.ESCAPE:input_box.removeAttr("aria-activedescendant");hide_dropdown();return!0;default:if(String.fromCharCode(event.which)){setTimeout(function(){do_search()},5)}
...
input_box.change()}
function hide_dropdown(){dropdown.hide();input_box.removeAttr("aria-describedby");if(selected_dropdown_item){$(selected_dropdown_item).removeClass(settings.classes.selectedToken)}
selected_dropdown_item=null}
function show_dropdown(){dropdown.css({position:"absolute"}).css('z-index',999).show()}
...
function show_dropdown_hint(){if(settings.hintText){var id_for_hint=hidden_input_id+"_autocomplete_hint";dropdown.html("<p class=\"notice\" id=\""+id_for_hint+"\">"+settings.hintText+"</p>");input_box.attr("aria-describedby",id_for_hint);show_dropdown()}}
...
function populate_dropdown(query,results){if(results&&results.length){dropdown.empty();input_box.removeAttr("aria-describedby");var dropdown_ul=$("<ul>").attr({"id":hidden_input_id+"_autocomplete_suggestions","role":"listbox"}).appendTo(dropdown).mouseover(function(event){select_dropdown_item($(event.target).closest("li"))}).mousedown(function(event){add_token($(event.target).closest("li"));return!1}).hide();$.each(results,function(index,value){var this_li=$("<li role=\"option\">"+highlight_term(escapeHTML(value.name),query)+"</li>").attr({"id":hidden_input_id+"_autocomplete_suggestion_"+index}).appendTo(dropdown_ul);if(index%2){this_li.addClass(settings.classes.dropdownItem)}else{this_li.addClass(settings.classes.dropdownItem2)}
