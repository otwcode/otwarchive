...
function populate_dropdown(query,results){if(results&&results.length){dropdown.empty();var dropdown_ul=$("<ul>").attr("role","listbox").appendTo(dropdown).mouseover(function(event){select_dropdown_item($(event.target).closest("li"))}).mousedown(function(event){add_token($(event.target).closest("li"));return!1}).hide();$.each(results,function(index,value){var this_li=$("<li>"+highlight_term(escapeHTML(value.name),query)+"</li>").attr({"id":hidden_input_id+"_autocomplete_suggestion_"+index,"role":"option"}).appendTo(dropdown_ul);if(index%2){this_li.addClass(settings.classes.dropdownItem)}else{this_li.addClass(settings.classes.dropdownItem2)}
