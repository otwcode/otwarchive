...
break;case KEY.ESCAPE:input_box.removeAttr("aria-activedescendant");hide_dropdown();return!0;default:if(String.fromCharCode(event.which)){setTimeout(function(){do_search()},25)}
...
var input_token=$("<li />").addClass(settings.classes.inputToken).attr({"aria-expanded":"false","aria-owns":hidden_input_id+"_autocomplete_dropdown","role":"combobox"}).appendTo(token_list).append(input_box);var dropdown=$("<div>").addClass(settings.classes.dropdown).insertAfter(input_box).hide();var li_data=$(input).val()||settings.prePopulate;if(settings.processPrePopulate&&$.isFunction(settings.onResult)){li_data=settings.onResult.call(hidden_input,li_data)}
...
input_box.change()}
function hide_dropdown(){dropdown.hide();if(!hidden_input_describedby){input_box.removeAttr("aria-describedby")}else{input_box.attr("aria-describedby",hidden_input_describedby)}
if(selected_dropdown_item){$(selected_dropdown_item).removeClass(settings.classes.selectedToken)}
input_token.attr("aria-expanded","false");selected_dropdown_item=null}
function show_dropdown(){dropdown.attr("id",hidden_input_id+"_autocomplete_dropdown").css({position:"absolute"}).css('z-index',999).show();input_token.attr("aria-expanded","true")}
...
function show_dropdown_hint(){if(settings.hintText){var id_for_hint=hidden_input_id+"_autocomplete_hint";dropdown.html("<p class=\"notice\" id=\""+id_for_hint+"\">"+settings.hintText+"</p>");input_box.attr("aria-describedby",function(index,value){if(!hidden_input_describedby){return id_for_hint}else{return hidden_input_describedby+" "+id_for_hint}});show_dropdown()}}
...
function populate_dropdown(query,results){if(results&&results.length){dropdown.empty();if(!hidden_input_describedby){input_box.removeAttr("aria-describedby")}else{input_box.attr("aria-describedby",hidden_input_describedby)};var dropdown_ul=$("<ul>").attr("role","listbox").appendTo(dropdown).mouseover(function(event){select_dropdown_item($(event.target).closest("li"))}).mousedown(function(event){add_token($(event.target).closest("li"));return!1}).hide();$.each(results,function(index,value){var this_li=$("<li>"+highlight_term(escapeHTML(value.name),query)+"</li>").attr({"id":hidden_input_id+"_autocomplete_suggestion_"+index,"role":"option"}).appendTo(dropdown_ul);if(index%2){this_li.addClass(settings.classes.dropdownItem)}else{this_li.addClass(settings.classes.dropdownItem2)}
...
input_box.attr("aria-activedescendant",item.attr("id"));item.addClass(settings.classes.selectedDropdownItem);selected_dropdown_item=item.get(0)}}
function deselect_dropdown_item(item){input_box.removeAttr("aria-activedescendant");item.removeClass(settings.classes.selectedDropdownItem);selected_dropdown_item=null}
