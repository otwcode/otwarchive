...
class Sanitize
  ...
  module Config
    ...
    ARCHIVE = freeze_config(
      ...
      attributes: {
        # The array for all MUST be duplicated in the CLASS_ATTRIBUTE config below
        # or else it will be overwritten entirely when merging
        all: ['align', 'title', 'dir'],
        'a' => ['href', 'name'],
        'blockquote' => ['cite'],
        'col' => ['span', 'width'],
    ...
    CLASS_ATTRIBUTE = freeze_config(
      ...
      attributes: {
        all: ['align', 'title', 'dir', 'class'],
      }
    )

    CSS_ALLOWED = merge(ARCHIVE, CLASS_ATTRIBUTE)
  end
